...
function selectCity(city) {
  document.querySelector(
    ".city-selector span"
  ).innerHTML = `<i class="fas fa-map-marker-alt"></i> ${city} <i class="fas fa-chevron-down"></i>`;
...
window.onclick = function (event) {
  if (
    !event.target.matches(".city-selector span") &&
    !event.target.matches(".city-selector span *")
  ) {
    document.getElementById("cityDropdown").classList.remove("active");
  }
  ...
  function startAutoSlide() {
    ...
    autoSlideInterval = setInterval(() => {
      const currentSlide = Math.floor(
        projectsGrid.scrollLeft / (projectsGrid.offsetWidth + 20)
      );
